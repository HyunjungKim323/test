...
void thread_main (void* a) {
    ...
    if(getpid() != pid) err_list[ERR_DIFF_PID]++;
...
int main (int argc, char** argv) {
    ...
    for(i = 0; i < n; i++){
        ...
        for(j = 0; j < i; j++) if(tids[j] == tids[i]) err_list[ERR_SAME_TID_EXIST]++;
    ...
    for(i = 0; i < n; i++){
        ...
        test_msg[4] = '0' + i;
        test_msg[5] = '\0';
        if(strcmp(args[i].msg, test_msg) != 0) err_list[ERR_DIFF_HEAP_SPACE]++;
        close(args[i].fd);
        open(args[i].msg, O_RDONLY);
        read(args[i].fd, file_msg, 6);
        if(strcmp(test_msg, file_msg) != 0) err_list[ERR_DIFF_FILE_DESC]++;
	}
    ...
    if(free_priv != frees()) err_list[ERR_RECLAIM_MEM_FAIL]++;
